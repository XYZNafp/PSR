...
def CountLetterMode(threshold):  # Mode where the test ends after 'threshold' inputs
    ...
    for i in range(1,threshold+1): # Defines a range
        ...
        input=input_tuple(palavra_shown=target_input, palavra_typed=typed_input, time=duration,letras_corretas=right_letters)
...
def TimeLetterMode(threshold):
    ...
    while timing < float(threshold):
        ...
        if  typed_input == chr(27):                    #Clicking on the space bar 
        ...
        input=input_tuple(palavra_shown=target_input, palavra_typed= typed_input,time = reaction_time, letras_corretas=right_letters ) #Stores the information from the test
...
def buildDict(inputs, abs_b4_time):
    ...
    for i in range(len(inputs)):
        if inputs[i].palavra_shown == inputs[i].palavra_typed:
            stat_dict['n_hits'] += 1
