...
import nltk # For the words mode
...
def TypeTest(threshold,TimedMode,WordMode):  # Mode where the test ends after 'threshold' inputs
    ...
    # This block will generate the target inputs for the 4 possible types of tests
    for k in range (1,threshold):
        if TimedMode == False:
            ...
            for i in range(1,threshold*4): # We assume that 4 inputs for each second is more than enough
                if WordMode == True:
                    target_input=0
                else:
                    target_input+=chr(random.randint(97,122))
        ...
        if TimedMode == False:
            for i in range(1,threshold+10): # Defines a range
                time_b4=time.time()
                print("Type "+ target_input[i])
                ...
                input=input_tuple(q=target_input[i],a=typed_input,t=duration)
